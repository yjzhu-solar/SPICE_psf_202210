...
spice_file = "~/work/spice_psf/spice_data/solo_L2_spice-n-ras_20210914T025031_V22_67109159-000.fits"

with fits.open(spice_file) as hdul:
    hdul.info()
    spice_hdr = hdul[2].header.copy()
    spice_dat = hdul[2].data[0].copy()
spice_dat = spice_dat.transpose([2,1,0]).astype(np.float32)

# cut out the region affected by WCSDVARR
spice_dat = spice_dat[:190,:,:]
spice_hdr["NAXIS1"] = 190
...
shift_vars = shift_holder(spice_dat, spice_hdr, fitter.__name__, save_dir='/cluster/home/zhuyin/work/spice_psf/spice_skew_20210914/',
                          linelist=linelist)

sv_initial = search_shifts(spice_dat, spice_hdr, xs_initial, ys_initial,
                           lsq_fitter, search_multi_thread=True, search_nthread=36,
                           yrange_plot_dir='/cluster/home/zhuyin/work/spice_psf/spice_skew_20210914/yrange_plots/',
...
plt.savefig(os.path.join('/cluster/home/zhuyin/work/spice_psf/spice_skew_20210914/yrange_plots/','varplot_'+labelstr+'.png'))
plt.close()
