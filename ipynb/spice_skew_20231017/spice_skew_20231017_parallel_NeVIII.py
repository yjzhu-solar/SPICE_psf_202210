...
with fits.open(spice_file) as hdul:
    hdul.info()
    spice_hdr = hdul[1].header.copy()
    spice_dat = hdul[1].data[0].copy()
...
shift_vars = shift_holder(spice_dat, spice_hdr, fitter.__name__, save_dir='/cluster/home/zhuyin/work/spice_psf/spice_skew_20231017/',
                          linelist=linelist)

sv_initial = search_shifts(spice_dat, spice_hdr, xs_initial, ys_initial,
                           lsq_fitter, search_multi_thread=True, search_nthread=36,
                           yrange_plot_dir='/cluster/home/zhuyin/work/spice_psf/spice_skew_20231017/yrange_plots/',
...
plt.savefig(os.path.join('/cluster/home/zhuyin/work/spice_psf/spice_skew_20231017/yrange_plots/','varplot_'+labelstr+'.png'))
plt.close()
